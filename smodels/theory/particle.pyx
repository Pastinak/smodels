...
class MultiParticle(Particle):
    ...
    def __new__(cls,label,particles,**kwargs):
        ...
        for obj in Particle.getinstances()[:]:
            if not isinstance(obj,MultiParticle):
                continue
            #Directly compare attributes, except for particles,label,id and cache
            objAttr = dict(obj.__dict__.items())            
            objAttr.pop('id',None)
            objAttr.pop('cache',None)
        ...
        newMultiParticle.cache.setToEqual ( [x.id for x in particles] )
    ...
    def __iadd__(self,other):
        ...
        if addParticles:
            ...
            self.cache.setToEqual ( [ x.id for x in self.particles ] )
