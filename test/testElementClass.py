...
class ElementTest(unittest.TestCase):
    ...
    def testElementMassComp(self):
        ...
        v1CompA = Vertex(inParticle=st1, outParticles=[n1,t,bbar])              
        b1Comp = Branch(vertices=[v0b,v1CompA])
        b2Comp = b2
        el2 = Element(branches=[b1Comp,b2Comp])
        
        print el1Comp.branches[1].vertices[0].stringRep()
        ...
        v1CompB = Vertex(inParticle=st1, outParticles=[n1,t])
        vComp = Vertex(inParticle=None, outParticles=[n1])
        b1Comp = Branch(vertices=[v0b,v1CompB])
        b2Comp = Branch(vertices=[vComp])
        el2 = Element(branches=[b1Comp,b2Comp])        
        self.assertEqual( el1Comp == el2, True) #Elements should be equal        
...
#         v0b = Vertex(inParticle=None, outParticles=[st1])
#         v1 = Vertex(inParticle=gluino, outParticles=[st1,t])
#         v2 = Vertex(inParticle=st1, outParticles=[n1,nue,nue])
#         
#         b1 = Branch(vertices=[v0,v1,v2])
#         b2 = Branch(vertices=[v0b,v2])
#         el1 = Element(branches=[b1,b2])
...
#         
#         v1Comp = Vertex(inParticle=None, outParticles=[st1])
#         b1Comp = Branch(vertices=[v1Comp,v2])
#         el2 = Element(branches=[b1Comp,b2])
#         
#         self.assertEqual( el1Comp == el2, True) #Elements should be equal
#         el1Comp = el1.massCompress(minmassgap = 1.*GeV)
#         self.assertEqual( el1Comp == el2, False) #Elements should not be equal
#         
#         el1Comp = el1.massCompress(minmassgap = 10.*GeV) #Fully compress
#         b1 = Branch(vertices=[v2])
#         b2 = Branch(vertices=[v2])
#         el3 = Element(branches=[b2,b1])
# 
#         self.assertEqual(el1Comp == el3, False) #Elements should not be equal        
